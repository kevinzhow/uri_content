...
interface UriContentPlatformApi {
  ...
  fun getContentRange(url: String, start: Long, length: Long, callback: (Result<ByteArray?>) -> Unit)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: UriContentPlatformApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.uri_content.UriContentPlatformApi.getContentRange$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val urlArg = args[0] as String
            val startArg = args[1] as Long
            val lengthArg = args[2] as Long
            api.getContentRange(urlArg, startArg, lengthArg) { result: Result<ByteArray?> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(UriContentPlatformApiPigeonUtils.wrapError(error))
              } else {
